...
import { Lib, merged, config } from '../../lib/lib';
...
const Main = (mySetting) => {
    ...
    class Index extends Component {
         ...
         */
        componentDidMount() {
            console.log("GetNextPage=====>componentDidMount start");
            console.log(this,this.props);
            console.log("GetNextPage======>componentDidMount end");
            this.redayDOM();
        }
        componentWillMount() {
            console.log("GetNextPage=====>componentWillMount start");
            console.log(this,this.props);
            console.log("GetNextPage======>componentWillMount end"); 
        }
        /**
    ...
    //这里传入action(),这里会将action里面的函数 那么每个定义
    //在该对象的函数都将被当作 Redux action creator，而且这个对象
    //会与 Redux store 绑定在一起，
    //其中所定义的方法名将作为属性名，合并到组件的 props 中
    return connect((state) => { return { state: state[setting.id], User: state.User } }, action())(Index); //连接redux
}
